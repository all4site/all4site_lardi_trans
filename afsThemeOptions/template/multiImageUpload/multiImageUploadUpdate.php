<div is="multiimageCreateUpdate" inline-template>
	<div>
		<div class="custom-file mb-3">
			...
			<label class="custom-file-label h-auto py-4 ustom-file-label-no-image" for="customFile">
			...
			<input type="file" class="custom-file-input" id="customFile" name="multiImageUpload"
			       @change="multipleImageUploadPreview"
			       @click="reset"
			       multiple>
		</div>

		<div class="col-md-12 mb-05 mt-7-05 px-0 d-flex blockUpload"
		     id="drop-zone"
		     @mouseover="dragImage">

			<div v-for="(file, index) in files" :key="file.name"
			     v-if="file.showLogo"
			     class="position-relative mb-3 mx-0  col  overflow-hidden drag-el"
			     :class="customClass"
			     :id="index">
				<i class="far fa-trash-alt trash-form-icon cursor-pointer trash-form-icon-file_upload" @click="deleteImage"></i>

				<img :src="file.logoPrev"
				     :name="file.name"
				     class="w-100 h-100 h-100 object-fit-cover rounded">

				<div v-if="file.error!=''" class="text-danger text-center image-error">
					{{file.error}}
				</div>
